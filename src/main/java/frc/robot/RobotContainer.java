...
import frc.robot.commands.hookCmds.HookLeftManualCmd;
import frc.robot.commands.hookCmds.HookLeftMotorDownPIDCmd;
import frc.robot.commands.hookCmds.HookLeftMotorUpPIDCmd;
import frc.robot.commands.hookCmds.HookRightManualCmd;
import frc.robot.commands.hookCmds.HookRightMotorDownPIDCmd;
import frc.robot.commands.hookCmds.HookRightMotorUpPIDCmd;
import frc.robot.commands.hookCmds.LineDownPIDCmd;
import frc.robot.commands.hookCmds.LineManualCmd;
...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    mainController.pov(270).whileTrue(new LineDownPIDCmd(hook));
     mainController.leftBumper().whileTrue(new HookLeftManualCmd(hook));
    mainController.leftTrigger().whileTrue(new HookLeftManualCmd(hook));
    mainController.rightBumper().whileTrue(new HookRightManualCmd(hook));
    mainController.rightTrigger().whileTrue(new HookRightManualCmd(hook));
    mainController.pov(90).whileTrue(new LineManualCmd(hook));
    mainController.pov(270).whileTrue(new LineManualCmd(hook));
